				...
				union power_supply_propval *val)
{
	...
	/* Workaround to support Type-C apapter without PD */
...
static void smblib_handle_typec_removal(struct smb_charger *chg)
{
	...
	rc = mblib_request_dpdm(chg, false);
	if (rc < 0)
		smblib_err(chg, "Couldn't disable DPDM rc=%d\n", rc);
...
static void smblib_handle_typec_cc_state_change(struct smb_charger *chg)
{
	...
	if (!chg->typec_present && chg->typec_mode != POWER_SUPPLY_TYPEC_NONE) {
		...
		/*
		 * D5X does not support wireless charging when OTG device is inserted,
		 * but E5 does support it by using external boost circuit for OTG.
		 */
